...
def output(res, courseName):
    ...
    if (not res.empty):
        ...
        res['questions'] = '{"attempt":'+res['attempts'].map(str)+',"mean":'+ res['mean'].map(str)+',"median":'+res['median'].map(str)+',"number_of_solutions":'+res['number_of_solutions'].map(str)+',"max_grade":'+res['max_grade'].map(str)+',"questions":'+res['questions'].map(str)+'}'
        res=res.drop(columns=['mean', 'attempts','median','number_of_solutions','max_grade'])
        res=res.groupby(['course_name','problem_header','problem_id','page','section','subsection'])['questions'].apply(','.join).reset_index()
        
        res['questions']='{"problem_id":"'+res['problem_id'].map(str)+'","attempts":['+ res['questions']+']}'
        res=res.drop(columns=['problem_id'])
        res=res.groupby(['course_name','problem_header','page','section','subsection'])['questions'].apply(','.join).reset_index()
        
        res['questions']='{"subsection":"'+res['subsection'].map(str)+'","problem_header":"'+res['problem_header'].map(str)+'","page":"'+res['page'].map(str)+ '","one_type_problems":['+ res['questions']+']}'
        res=res.drop(columns=['page','problem_header','subsection'])
