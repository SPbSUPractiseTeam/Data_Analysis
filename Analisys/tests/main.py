...
def Execute(cursor, courseName):
    ...
    if (data):
        ...
        tmpdf=df.groupby(['course_name','page','problem_header','problem_id','attempts']).agg({'max_grade': [np.max], 'execute_time': [np.min]})        
        ...
        listForQuestions = pd.DataFrame(df, columns=['course_name','page','problem_header','problem_id','attempts'])
        ...
        res=pd.merge(avg, perc, on=['course_name','page','problem_header','problem_id','attempts'], how='outer')
        res=pd.merge(res, tmpdf, on=['course_name','page','problem_header','problem_id','attempts'], how='outer')
        res=res.sort_values(by=['execute_time'])
        res=res.drop(columns=['execute_time'])
    else:
        res=""
